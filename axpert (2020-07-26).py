...
def on_connect(client, userdata, flags, rc):
    client.subscribe('Inverter/InputPower')
    if rc==0:
        print("connected OK")
    else:
...
def on_message(client, userdata, msg):  # The callback for when a PUBLISH message is received from the server.
    ...
    if str(msg.topic) == 'Inverter/InputPower':
        time.sleep(3)
        if str(msg.payload) == 'Grid':
            print('Going to Grid')
            get_data("POP00", 0)
            switch = 0
        if str(msg.payload) == 'Solar':
            get_data("POP02", 0)
            print('Going to Solar')
            switch = 1
...
client.username_pw_set("mqtt", "4a2fc629")
...
def main():
    ...
    totalPow = 0
    PVPower = 0
    #response = serial_command("QBV", usb0)
    #print("QBV - " + response)
    #response = serial_command("Q1", usb0)
    #print("Q1 - " + response)
    #response = serial_command("POP00", usb0)
    #print("POP00 - " + response)
    #time.sleep(5)
    #response = serial_command("QMOD", usb0)
    #print(response)
    #time.sleep(5)
    #response = serial_command("POP02", usb0)
    #print("POP02 - " + response)
    #time.sleep(5)
    #response = serial_command("QMOD", usb0)
    #print(response)
    endtime = time.time()
    ...
    while True:
        ...
        for inverter in range(0, 1):
           ...
           # print(switch)
            ...
            totalPow = totalPow + float(nums_global[4]) * float(deltat) / 3600 / 1000
            PVPower = PVPower + float(nums_global[12]) * float(nums_global[13]) / 3600 / 1000
            ...
            #            print(totalPow)
            #print(deltat)
            #Wattsec = Wattsec + float(nums_global[4])*float(deltat)/3600*1.06409#
            #VAsec = VAsec + float(nums_global[5])*float(deltat)/3600
            Ah=Ah+(float(nums_global[9]) - float(nums_global[15])) * float(deltat) / 3600

            if (float(nums_global[8]) >= 51.3) and (float(nums_global[9] < 3)):
                Ah = 100
            if Ah > 100:
                Ah = 100
            client.publish('/home-assistant/battery/SOC', round(Ah, 1))
            client.publish('/home-assistant/battery/TotalOutputPower', round(totalPow,1))
            client.publish('/home-assistant/battery/TotalPVPower', round(PVPower,1))

            client.publish('/home-assistant/battery/OutputPower', round(float(nums_global[5]), 1))
            client.publish('/home-assistant/battery/BattVoltage', round(float(nums_global[8]), 1))
            client.publish('/home-assistant/battery/PVPower', round(float(nums_global[12])*float(nums_global[8]), 1))
            client.publish('/home-assistant/battery/BattPower', round(float(nums_global[8])*(float(nums_global[9])-float(nums_global[15])), 1))

#            Output_Power, Battery_Voltage, PV_Input_Current, Charge_Current, Discharge_Current,
#            (nums[5], nums[8], nums[12], nums[9], nums[15], PVpower))

            #BattEng = BattEng + float(nums_global[8])*(float(nums_global[9]) - float(nums_global[15])) * float(deltat) / 3600
            #print(BattEng)
            #print(Wattsec)
            #print(VAsec)
            #print(Ah)
